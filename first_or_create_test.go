...
func TestAttrsAssignSurviveSessionClone(t *testing.T) {
	...
	t.Run("FirstOrCreate Assign after WithContext", func(t *testing.T) {
		u := attrsAssignsUser{Name: "jinzhu"}
		result := db.Where("name = ?", "jinzhu").Assign("age", 66).WithContext(ctx).FirstOrCreate(&u)
		if result.Error != nil {
	...
	t.Run("FirstOrCreate Attrs after WithContext", func(t *testing.T) {
		u := attrsAssignsUser{Name: "newbie"}
		result := db.Where("name = ?", "newbie").Attrs("age", 42).WithContext(ctx).FirstOrCreate(&u)
		if result.Error != nil {
			t.Fatalf("unexpected error: %v", result.Error)
		}
		if u.Age != 42 {
	...
	t.Run("FirstOrInit Assign after WithContext", func(t *testing.T) {
		u := attrsAssignsUser{Name: "jinzhu"}
		result := db.Where("name = ?", "jinzhu").Assign("age", 77).WithContext(ctx).FirstOrInit(&u)
		if result.Error != nil {
			t.Fatalf("unexpected error: %v", result.Error)
		}
		if u.Age != 77 {
