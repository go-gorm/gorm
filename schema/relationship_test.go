...
// assertPlainForeignKeyType verifies that a foreign key column type does not
// carry a value-generating clause inherited from the referenced primary key.
func assertPlainForeignKeyType(t *testing.T, f *schema.Field) {
	t.Helper()
	if f == nil {
		t.Fatalf("foreign key field not found")
		return
	}
	lower := strings.ToLower(string(f.DataType))
	for _, clause := range []string{"auto_increment", "autoincrement", "identity", "generated", "serial"} {
		if strings.Contains(lower, clause) {
			t.Errorf("foreign key %q must not inherit the %q clause from its referenced primary key, got DataType %q", f.Name, clause, f.DataType)
		}
	}
}

// TestForeignKeyDataTypeWithIdentityPrimaryKey makes sure a primary key whose
// `type:` tag carries a value-generating clause (e.g. a Postgres identity
// column, `bigint generated by default as identity`) does not propagate that
// clause to the foreign keys that reference it. Otherwise every referencing
// foreign key would wrongly own its own identity/sequence.
//
// https://github.com/go-gorm/gorm/issues/7191
func TestForeignKeyDataTypeWithIdentityPrimaryKey(t *testing.T) {
	type Company struct {
		ID uint `gorm:"primarykey;type:bigint generated by default as identity"`
	}
	...
	// The primary key itself keeps its `type:` tag verbatim.
	if dt := string(s.FieldsByName["ID"].DataType); dt != "bigint generated by default as identity" {
		t.Errorf("primary key User.ID should keep its type tag, got DataType %q", dt)
	}

	// belongs-to and self-referential foreign keys live on the User schema.
	assertPlainForeignKeyType(t, s.FieldsByName["CompanyID"])
	assertPlainForeignKeyType(t, s.FieldsByName["ManagerID"])

	// the polymorphic foreign key lives on the referenced (Toy) schema.
	toyRel := s.Relationships.Relations["Toy"]
	if toyRel == nil {
		t.Fatalf("expected a polymorphic Toy relationship")
	}
	assertPlainForeignKeyType(t, toyRel.FieldSchema.FieldsByName["OwnerID"])

	// the many2many foreign keys live on the generated join-table schema.
	languagesRel := s.Relationships.Relations["Languages"]
	if languagesRel == nil || languagesRel.JoinTable == nil {
		t.Fatalf("expected a many2many Languages relationship with a join table")
	}
	for _, f := range languagesRel.JoinTable.Fields {
		assertPlainForeignKeyType(t, f)
	}
}
