...
func (schema *Schema) buildMany2ManyRelation(relation *Relationship, field *Field, many2many string) {
	...
	for idx, ownField := range ownForeignFields {
		...
		joinTableFields = append(joinTableFields, reflect.StructField{
			...
			Tag:     joinTableFieldTag(ownField),
		})
	}
	...
	for idx, relField := range refForeignFields {
		...
		if _, ok := fieldsMap[joinFieldName]; !ok {
			...
			joinTableFields = append(joinTableFields, reflect.StructField{
				...
				Tag:     joinTableFieldTag(relField),
			})
		}
	}
...
// copyableDataType reports whether a referenced primary key's data type may be
// reused verbatim for the foreign key that points at it.
//
// A foreign key only needs to store the same value as the referenced key, so it
// must not inherit column clauses that make a key auto-generate its own value.
// Copying such clauses creates a foreign key that, for example, owns its own
// identity/sequence (e.g. `bigint GENERATED BY DEFAULT AS IDENTITY`, `bigserial`,
// MySQL `AUTO_INCREMENT`, SQLite `AUTOINCREMENT`). When the type cannot be copied
// the foreign key keeps the data type derived from its own struct field instead.
// See issues #5222, #7191.
func copyableDataType(str DataType) bool {
	lowerStr := strings.ToLower(string(str))
	for _, s := range []string{"auto_increment", "autoincrement", "primary key", "identity", "generated", "serial"} {
...
// joinTableFieldTag derives the struct tag of a join-table foreign key from the
// referenced field it mirrors. Settings that must not propagate to the join
// table are stripped. In particular, an explicit `type:` is dropped when it
// carries a value-generating clause (see copyableDataType) so the join-table
// foreign key falls back to the data type of its own Go field instead of
// inheriting the referenced key's identity/sequence. See issue #7191.
func joinTableFieldTag(field *Field) reflect.StructTag {
	removedSettings := []string{"column", "autoincrement", "index", "unique", "uniqueindex"}
	if !copyableDataType(field.DataType) {
		removedSettings = append(removedSettings, "type")
	}
	return removeSettingFromTag(appendSettingFromTag(field.StructField.Tag, "primaryKey"), removedSettings...)
}
