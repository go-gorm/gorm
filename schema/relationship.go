...
// `AUTO_INCREMENT`). When the type cannot be copied the foreign key keeps the
// data type derived from its own struct field instead. See issues #5222, #7191.
func copyableDataType(str DataType) bool {
	lowerStr := strings.ToLower(string(str))
	for _, s := range []string{"auto_increment", "primary key", "identity", "generated", "serial"} {
